...
io.on("connection", (socket) => {
 ...
 socket.on("blockedCountries:check", (country) => {
  console.log("🌍 Country check received:", country);
  console.log("🚫 Blocked countries:", globalBlockedCountries);

  const normalizedCountry = String(country || "").trim().toLowerCase();

  const isBlocked = globalBlockedCountries.some(
    c => String(c || "").trim().toLowerCase() === normalizedCountry
  );

  console.log("🔎 Country:", normalizedCountry);
  console.log("🚫 Is blocked:", isBlocked);

  socket.emit("blockedCountries:checkResult", {
    isBlocked,
    country
  });
});
